...
def mfrcnn_pred(out, nb_classes=81, pooling_regions=7, dim_in=2048):
    # out = Input(shape=(pooling_regions, pooling_regions, dim_in)) # from above frcnn 7 *7*2048
    out_mask = Conv2DTranspose(256, (2, 2), strides=2, name = 'mask_14')(out)
    # Flatten the convlutional layer and connected to 2 FC and 2 dropout
    out_mask = Conv2D(nb_classes-1, (3,3), padding='same', activation='sigmoid')(out_mask)
    return out_mask


def mask_frcnn_pred(nb_classes=81, pooling_regions=7, dim_in=256):
    ...
    out_mask = mfrcnn_pred(out, nb_classes=81, pooling_regions=7, dim_in=2048)
    ...
    return Model(out_roi_pool, [out_class, out_regr, out_mask])

# # model = frcnn_pred (nb_classes=81, pooling_regions=7, dim_in=256)
# # model.summary()
# # model.save('Resnet50_C4_faster_head.hdf5')



# def mfrcnn_pred(nb_classes=81, pooling_regions=7, dim_in=2048):
#     out = Input(shape=(pooling_regions, pooling_regions, dim_in)) # from above frcnn 7 *7*2048
#     out_mask = Conv2DTranspose(256, (2, 2), strides=2, name = 'mask_14')(out)
#     # Flatten the convlutional layer and connected to 2 FC and 2 dropout
#     out_mask = Conv2D(nb_classes-1, (3,3), padding='same', activation='sigmoid')(out_mask)
#     return Model(out, [out_mask])



# # model = mfrcnn_pred ( nb_classes=81, pooling_regions=7, dim_in=2048)
# # model.summary()
# # model.save('Resnet50_C4_mask_head.hdf5')
