...
from .mfrcnn_head import mask_frcnn_pred
...
def create_feature_map(input_shape):
    ...
    return model.input, model.get_layer("activation_40").output
...
def create_rpn(input_shape=(480, 640, 3),num_anchors=15):
    ...
    inputs, base_feature = create_feature_map(input_shape)
...
def create_detectron_mfrcnn_resnet50v1c4(shape):
    model1 = create_rpn(shape)
    model2 = mask_frcnn_pred(nb_classes=81, pooling_regions=7, dim_in=1024) # include faster and mask
...
#     rpn_model.save("R-50-C4_800x640.hdf5")

#     K.clear_session()
#     rpn_model = rpn_create((640, 480, 3))
#     rpn_model.save("R-50-C4_640x480.hdf5")

#     K.clear_session()
#     rpn_model = rpn_create((480, 320, 3))
#     rpn_model.save("R-50-C4_480x320.hdf5")
