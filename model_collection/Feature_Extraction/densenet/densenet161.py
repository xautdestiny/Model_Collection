...
def DenseNet(nb_dense_block=4, growth_rate=48, nb_filter=96, reduction=0.0, dropout_rate=0.0, weight_decay=1e-4, classes=1000, weights_path=None):
    ...
    x = Convolution2D(nb_filter, (7, 7), subsample=(2, 2), name='conv1', use_bias=False, padding='same')(img_input)
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4):
    ...
    x = Convolution2D(nb_filter, (3, 3), name=conv_name_base+'_x2', use_bias=False, padding='same')(x)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    for i in range(nb_layers):
        ...
        concat_feat = Concatenate(name='concat_'+str(stage)+'_'+str(branch), axis=-1)([concat_feat, x])
...
def create_densenet161(input_shape=(224, 224, 3)):
