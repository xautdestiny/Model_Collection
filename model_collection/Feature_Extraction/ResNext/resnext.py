...
def ResNextImageNet(input_shape=None, depth=[3, 4, 23, 3], cardinality=64, width=4, weight_decay=5e-4,
...
def __grouped_convolution_block(input, grouped_channels, cardinality, strides, weight_decay=5e-4):
    ...
    if cardinality == 1:
        ...
        x = Conv2D(grouped_channels, (3, 3), padding='same', use_bias=False, strides=(strides, strides),
                   kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay))(init[0])
        x = BatchNormalization(axis=channel_axis)(x)
        x = Activation('relu')(x)
        return x
...
def __bottleneck_block(input, filters=64, cardinality=8, strides=1, weight_decay=5e-4):
    ...
    for i in range (cardinality):
    	x = Conv2D(grouped_channels, (1, 1), padding='same', use_bias=False, kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay))(input)
    	x = BatchNormalization(axis=channel_axis)(x)
    	x = Activation('relu')(x)
    	xs.append(x)


    x = __grouped_convolution_block(xs, grouped_channels, cardinality, strides, weight_decay)
...
#    model = ResNextImageNet((800, 600, 3), cardinality= 32, width= 8)
#    model.summary()
#    model.save("rexnext101_.hdf5")